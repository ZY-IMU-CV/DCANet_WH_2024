...
class ConvBnAct(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=1, stride=1, padding=0, dilation=1, groups=1,
        ...
        self.bn_city=norm2d(out_channels)
        self.bn_cam = norm2d(out_channels)
        self.bn_bdd = norm2d(out_channels)
        if apply_act:
            self.act=activation()
        else:
            self.act=None
...
class DBlock(nn.Module):
    def __init__(self, in_channels, out_channels, dilations, stride):
        ...
        self.bn1_city=norm2d(out_channels)
        self.bn1_cam = norm2d(out_channels)
        self.bn1_bdd = norm2d(out_channels)
        ...
        self.bn2_city=norm2d(out_channels)
        self.bn2_cam = norm2d(out_channels)
        self.bn2_bdd = norm2d(out_channels)
        self.act2=activation()
        self.conv3=nn.Conv2d(out_channels, out_channels,kernel_size=1,bias=False)
        self.bn3_city = norm2d(out_channels)
        self.bn3_cam = norm2d(out_channels)
        self.bn3_bdd = norm2d(out_channels)
...
class eleven_Decoder0(nn.Module):#Sum+Cat
    def __init__(self, channels):
        ...
        self.classifier_cam = nn.Conv2d(64, 12, (1, 1))
