...
def train():
    ...
    log_path = "./log_file/crossdataset_test2"


    model = RegSeg()

    total_iterations = min(len(train_loader_city),len(train_loader_cam),len(train_loader_bdd))*max_epochs
    ...
    for epoch in range(epoch_start, epochs):
        ...
        for x_city, x_cam,x_bdd in zip(train_loader_city, train_loader_cam,train_loader_bdd): 
            ...
            lr_scheduler.step()
            losses += loss.item()
        print(losses/int(min(len(train_loader_city),len(train_loader_cam),len(train_loader_bdd))))
        with open(log_path, "a") as f:
            f.write(f"{epoch}\n")
            f.write(f"{losses/int(min(len(train_loader_city),len(train_loader_cam),len(train_loader_bdd)))}\n")
