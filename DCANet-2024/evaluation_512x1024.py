...
if __name__ =="__main__":
    ...
    config["model_name"] = "tentwo3whnum01234selfvits5b2rightddlight1_decoderRDDwhpaper640s6"
    config["save_best_path"] = './pth_file/best_WH_DW_tentwo3whnum01234selfvits5b2rightddlight1_decoderRDDwhpaper640s6.pth'
    class_num = 19
    hist = np.zeros((class_num, class_num))
    val_data = val_data.get_cityscapes('./cityscapes_dataset/', (512, 1024), (512, 1024), 0.5, 'val', 4)
    #val_data = val_data.get_cityscapes('./cityscapes_dataset/',(768,1536),(768,1536),0.5,'val',4) #计算fps图像大小，也是计算验证集miou图像大小
    ...
    #inpute = torch.randn(1,3,768,1536)     #计算GFLOPS与params时的图像大小
    inpute = torch.randn(1, 3, 512, 1024)
    ...
    with torch.no_grad():
        ...
        net1.load_state_dict(torch.load(config["save_best_path"], 'cuda')['model']) #miou

        net1.eval()
        comfort = ConfusionMatrix(19, [])   #miou
        comfort = evaluate(net1, val_data, 'cuda', comfort, True, 5, 500)   #miou
        print(comfort)     #miou
