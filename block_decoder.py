...
class Exp2_Decoder26_selfattention(nn.Module):
    def __init__(self, num_classes, channels):
        super().__init__()
        channels4,channels8,channels16=channels["4"],channels["8"],channels["16"]
        self.head16=ConvBnAct(channels16, 128, 1)
...
class STDC_docker_wh(nn.Module):
    def __init__(self):
        super(STDC_docker_wh, self).__init__()
        self.avg_conv = nn.Sequential(nn.Conv2d(1024,128,(1,1),(1,1)),nn.ReLU(inplace=True))
        self.arm16 = AttentionRefinementModule(256, 128)
