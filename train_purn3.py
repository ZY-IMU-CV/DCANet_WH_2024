...
def train_one(config):
    ...
    purnrate = config["purnrate"]
    if not config["resume"]:
        with open(log_path,"a") as f:
            f.write(f"{config}\n")
            f.write(f"run: {run}\n")
            f.write(f"purn rate: {purnrate}\n")

    keep_masks = SNIP(model, 1-purnrate, train_loader, device)  # TODO: shuffle?
...
if __name__=='__main__':
    ...
    config["log_path"] = "./log_file/DW_tentwo1_purn4"
    config["save_best_path"] = './pth_file/best_DW_tentwo1_purn4.pth'
    config["purnrate"] = 0.1
